...
import { openingHoursSchema, postalAddressSchema, breadcrumbList } from '@/lib/schemaHelpers';
...
export default function Home() {
  ...
  const homeSchemas = useMemo(() => {
    const scooters = ['Activa Electric Scooter', 'Single Light Electric Scooter', 'Double Light Electric Scooter', 'Zoom Electric Scooter'];
    return [
      breadcrumbList([{ name: 'Home', path: '/' }]),
      {
        ...
        hasOfferCatalog: {
          ...
          itemListElement: scooters.map((name) => ({
            '@type': 'Offer',
  ...
  return (
    <>
      <SEO
        ...
        path="/"
        description="Buy premium low-speed electric scooters in Berhampore, West Bengal. No licence, no registration. Activa, Single Light, Double Light, Zoom models. 1 year warranty."
        jsonLd={homeSchemas}
        titleTemplate={false}
      />
