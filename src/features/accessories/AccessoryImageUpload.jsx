...
import { uploadAccessoryImage } from './accessoryService';
import { cn } from '@/lib/utils';

const MAX_MB = 8;
const ACCEPT = 'image/jpeg,image/png,image/webp';

export function AccessoryImageUpload({ images = [], onChange, accessoryId }) {
  ...
  const addFiles = async (files) => {
    ...
    setUploading(true);
    try {
      const urls = await Promise.all(valid.map((f) => uploadAccessoryImage(f, accessoryId)));
