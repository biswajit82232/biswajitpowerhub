/**
 * Site photo slots — persisted in localStorage; files uploaded to Supabase Storage when configured.
...
export async function uploadSitePhotoFile(file, folder = 'site') {
  let uploadFile = file;
  try {
    if (folder === 'hero') uploadFile = await resizeImageFile(file, 1200, 600);
    else if (folder.startsWith('gallery') || folder === 'about') uploadFile = await resizeImageFile(file, 800, 600);
    else if (folder.startsWith('model')) uploadFile = await resizeImageFile(file, 600, 400);
  } catch {
    uploadFile = file;
  }
