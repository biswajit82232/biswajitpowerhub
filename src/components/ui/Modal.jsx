      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            tabIndex={-1}
            initial={{ opacity: 0, y: centered ? 12 : 40, scale: 0.96 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: centered ? 12 : 40, scale: 0.96 }}
            transition={{ type: 'spring', stiffness: 320, damping: 30 }}
            className={cn(
              'relative z-10 w-full bg-surface shadow-card-hover outline-none',
              'max-h-[min(92vh,36rem)] overflow-y-auto',
              centered ? 'mx-auto rounded-3xl' : 'rounded-t-3xl sm:rounded-3xl',
              sizes[size],
              className,
            )}
          >
